$(window).load(function(){
	...
	function init(){
		...
		if(!MSIE){
			sliders.not(sliders.eq(slidNumberNew)).animate({"left":'-950px'}, 0)
		}else{
	...
	function changeImageNext(){
		anomationStop=false;
		if(!MSIE){
			sliders.eq(slidNumberOld).css({"background":"none"}).animate({"left":'-950px'}, 500)
    ...
    /////////////////////////////
		...
		if(!MSIE){
			sliders.eq(slidNumberNew).css({"background":"none",'left':'-950px'}).animate({"left":'0px'}, 500, function(){anomationStop=true;})
		}else{
			sliders.eq(slidNumberNew).fadeTo(500,1, function(){anomationStop=true;});
		}
