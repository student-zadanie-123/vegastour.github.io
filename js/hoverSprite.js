			...
			function verificationPageHandler(){
		 	...
		 	function init(){
				...
				function overHandler(){
					if(!MSIE){
						$(this).find(".overIcon").stop().animate({opacity:1}, 500, "easeOutCubic")
				...
				function outHandler(){
					if(!MSIE){
						$(this).find(".overIcon").stop().animate({opacity:0}, 500, "easeOutCubic")
						$(this).find(".outIcon").stop().animate({opacity:1}, 500, "easeOutCubic")
					}else{
						item.find(".overIcon").css({"visibility":"hidden"})
